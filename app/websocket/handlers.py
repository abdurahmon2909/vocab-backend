            ...
            if event == "join_duel":
                ...
                if room_id:
                    ...
                    await manager.broadcast_to_room(
                        {"event": "duel_started", "room_id": room_id, "questions": questions, "total_questions": 20},
                        room_id
                    )
                else:
                    await manager.send_personal_message({"event": "duel_queue", "status": "waiting"}, user_id)
                ...
                if result.get("status") == "ready":
                    ...
                    await manager.send_personal_message(
                        {"event": "team_queue", "status": result["status"], "team": result.get("team")},
                        user_id
                    )
                ...
                await manager.send_personal_message({"event": "left_queue", "queue_type": queue_type}, user_id)
